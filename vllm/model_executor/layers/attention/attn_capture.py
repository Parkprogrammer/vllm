...
class AttentionCapture:
    ...
    def buffer_query(
        ...
        K is NOT buffered — it is read from KV cache at capture time (request completion).
        """
        ...
        except Exception: return # This error may occur due to gpu memory access issues
        ...
        for i in range(query.shape[0]):
            ...
            buf_key = (layer_idx, slot_id)
            if buf_key not in self.q_buffer: self.q_buffer[buf_key] = []

            q_tok = (query_cpu[i].to(torch.float16) # cast to float16 for uniform buffer dtype
                     if query_cpu[i].dtype != torch.float16
                     else query_cpu[i])
            self.q_buffer[buf_key].append(q_tok)
    ...
    def capture(
        ...
        for layer_idx in layers:
            ...
            # Collect Q tensors from the ordered slots
            ...
            # Reads the KV Cache right after the request is finished before the KV cache is freed.
            # From the aligned group, extract the keys for the current request's slot_id
            ...
            tok_idx, q_list, k_list = self._filter_compatible_qk(
                tok_idx, q_list, k_list)
            if not q_list: continue

            # Compute Q*K^T attention scores on gpu, since the filter checks for device alignments
            # Q tensors were detached to CPU in buffer_query, so GPU computation here is safe.
            ...
            # Remove processed slots from q_buffer to prevent cross-request contamination.
    ...
    def cleanup_request_buffers(
        ...
        to prevent stale data leaking into future requests.
        """
        ...
        keys = [k for k in self.q_buffer if k[1] in remove]
        for k in keys:
            del self.q_buffer[k]
...
def resolve_target_layers(
    req_state, default_layers: Set[int] | None,
) -> Set[int]:
    ...
    if req_state.sampling_params and req_state.sampling_params.extra_args:
        layers_str = req_state.sampling_params.extra_args.get(
            'attn_capture_layers')
...
def extract_k_from_kv_cache(
    ...
    if kv_cache.ndim == 5 and shape[0] == 2:
        ...
        raise ValueError(
            f"Unsupported KV cache layout: ndim={kv_cache.ndim} "
            f"shape={list(shape)}")
...
def compute_qk_attention(
    ...
    if hk == hq:
        ...
        k_m = k.index_select(
            0, torch.arange(hq, device=k.device) // (hq // hk))
    else:
        idx = torch.clamp(torch.floor(
                torch.arange(hq, device=k.device) * (hk / hq)).long(),
            0, hk - 1)
...
def build_token_meta(
    ...
    *,
    ...
    for r in raw_vision:
        ...
        else: vision_ranges[-1]["end"] = max(vision_ranges[-1]["end"], r["end"])
    ...
    ordered_len = int(ordered_slots_len if ordered_slots_len is not None
                            else len(token_idx))
