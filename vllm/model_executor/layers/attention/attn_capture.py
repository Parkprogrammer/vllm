...
with zero generation overhead. Attention scores (Q*K) are computed
on the GPU at request-free time and delivered via shared memory.
...
def build_token_meta(
    ...
    *,
    ...
    # Collect vision ranges from multi-modal features
    ...
    # Complement: language ranges within prompt
    ...
    if cursor < prompt_len:
        language_ranges.append({"start": cursor, "end": prompt_len})
    ...
    prompt_boundary_with_offset = (
        bisect_left(token_idx_shifted, prompt_len)
        if token_idx_shifted else None)
...
def _collect_qk_pairs(
    ...
    for si, sid in enumerate(slots):
        q_buf = q_buffer.get((layer_idx, sid))
...
class AttentionCapture:
    ...
    def buffer_query(
        ...
        for i in range(query.shape[0]):
            ...
            q_tok = (query_cpu[i].to(torch.float16)
    ...
    def cleanup_request_buffers(
        ...
        Must be called for ALL finished requests (regardless of capture
        flag) to prevent stale data leaking into future requests.
        """
